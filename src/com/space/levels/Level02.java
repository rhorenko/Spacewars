...
public class Level02 extends GameSurface {
	public void startPositions() {
		...
		if (b4 == 0)
			ships[b4][k].setPosition((int) (shipWidth + rand.nextInt(171)
					* scale), (int) (-50.0D - k * objectPadding
					* (1D - 0.29999999999999999D * (2 - k % 3))));
		while (true) {
			do
				while (true) {
					while (true)
					...
					ships[b4][k]
							.setPosition(
									(int) (shipWidth + rand.nextInt(141)
											* scale),
									(int) (-200.0D - k
											* objectPadding
											* (1D - 0.29999999999999999D * (2 - k % 3))));
			...
			while (b4 != 2);
			ships[b4][k].setPosition((int) (30 + shipWidth + rand.nextInt(211)
					* scale), -300);
		}
	...
	public void updatePositions() {
		...
		while (true) {
			if (b >= 5) {
				...
				while (true) {
					if (j >= numberOfBees)
						;
					beeAngle[j] = (int) Math.round(beeAngle[j]
							+ 2.6000000000000001D * beeDirection[j]);
					bees[j].setPosition(
							bees[j].getLeft()
									- (int) Math.round(Math.sin(Math
											.toRadians(180 + beeAngle[j]))
											* (tempX + f / 8.5F)),
							bees[j].getTop()
									+ 2
									* tempY
									/ 3
									+ Math.round(f / 3.5F)
									- (int) Math.round(Math.cos(Math
											.toRadians(beeAngle[j])) * tempY));
					++j;
				}
			}
			i = 0;
		...
		if (smashed[b][i] == false) {
			...
			if (b != 0)
				break label431;
			ships[b][i].setPosition(
					Math.round(ships[b][i].getLeft() + f * shipDirection[b][i]
							* tempX), ships[b][i].getTop()
							+ (int) (tempY * scale * (1F + f / 3.0F)));
			double d2 = Math.atan2(f * shipDirection[b][i] * tempX, tempY
					* scale * (1F + f / 3.0F));
		...
		while (true) {
			do
				while (true) {
					...
					label431: if (b != 1)
						break;
					ships[b][i].setPosition(
							Math.round(ships[b][i].getLeft()
									+ shipDirection[b][i] * tempX * (1F + f)),
							ships[b][i].getTop()
									+ (int) (tempY * scale * (1F + f / 3.0F)));
					double d1 = Math.atan2(shipDirection[b][i] * tempX
							* (1F + f), tempY * scale * (1F + f / 3.0F));
			...
			if (ships[b][i].getLeft() < 0)
				shipAngle[b][i] = 110;
			shipAngle[b][i] = (int) Math.round(shipAngle[b][i]
					+ 3.6000000000000001D * shipDirection[b][i]);
			if ((shipAngle[b][i] > 270) || (shipAngle[b][i] < 90)) {
				int[] arrayOfInt = shipDirection[b];
				arrayOfInt[i] = (-1 * arrayOfInt[i]);
			}
			ships[b][i].setPosition(
					(int) (ships[b][i].getLeft() + tempX
							* Math.sin(Math.toRadians(shipAngle[b][i]))),
					(int) (ships[b][i].getTop() - tempY
							* Math.cos(Math.toRadians(shipAngle[b][i]))));
		}
	}
}