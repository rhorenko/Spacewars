...
public class GameActivity extends Activity {
			...
			while (true) {
				while (true) {
					while (true) {
						...
						label486: localClass = LevelsList
								.getLevel(InitialView.rand
										.nextInt(LevelsList.numberOfLevels));
					}
					label502: numberFrames[j] = getApplicationContext()
							.getResources().getDrawable(2130837627 + j);
					++j;
				}
				label531: numbers[b] = new ImageView(getApplicationContext(),
						null);
				numbers[b].bringToFront();
				RelativeLayout.LayoutParams localLayoutParams3 = new RelativeLayout.LayoutParams(
						(int) (35.0F * f), (int) (35.0F * f));
	...
	public void pause(Boolean paramBoolean) {
		...
		if ((pauseWarning.getVisibility() == 0)
		...
		while (true) {
			...
			if ((backgroundMp != null)
					&& (!(backgroundMpErrored.booleanValue()))
					&& (backgroundMp.isPlaying()))
				backgroundMp.stop();
	...
	public void update()
  {
	...
	public void updateScores() {
		...
		while (true) {
			if (j >= Math.min(
					(int) Math.ceil(Math.log10(0.10000000000000001D + i)), 5)) {
				b = 0;
				if (b < 5)
					break;
				if (i < 10) {
			...
			arrayOfInt[j] = ((int) (i / Math.pow(10.0D,
					-1 + (int) Math.ceil(Math.log10(0.10000000000000001D + i))
							- j)) % 10);
			++j;
		}
		if (arrayOfInt[b] != -1) {
	...
	public GameActivity() {// Constructor
		scoreBar = null;
		pauseButton = null;
		pauseWarning = null;
		bringItOnHome = false;
	...
	private void createLevel(Class<?> pClass) {
		Constructor constructor = pClass.getConstructors()[0];
		try {
			if (gameSurface != null) {
				surfaceView.surfaceDestroyed(null);
				surfaceView.stop();
			}
			gameSurface = ((LevelInterface) constructor
					.newInstance(new Object[0]));
			Context context = getApplicationContext();
			if (surfaceView == null)
				surfaceView = new GameSurfaceView(context);
			((GameSurface) gameSurface).setup(context, surfaceView);
			surfaceView.setSurfaceSize(context.getResources()
	...
	public void onBackPressed() {
		...
		((GameSurface) gameSurface).doStop();
		SharedPreferences.Editor localEditor = getSharedPreferences("AppData",
				0).edit();
		localEditor.putInt("State", 3);
		localEditor.commit();
	...
	public void updateLifes() {
		...
		int i = sharedPreferences.getInt("Lifes", 0);
